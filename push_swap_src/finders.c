...
t_target	ft_find_biggest(t_node *head, int stack_size)
{
	t_target	ret;
	int		biggest;
	int		i;
	...
	while (head)
	{
		...
		{
			ret = ft_count_target(head->value, i, stack_size);
			biggest = ret.target;
		}
		head = head->next;
		i++;
...
t_target	ft_find_smallest_pair(t_node *head, int stack_size)
{
	t_target	ret;
	int		smallest;
	int		i;
...
diff with ft_diff */
t_target	ft_find_pair_for_b(int stack_b, t_node *head, int list_size)
{
	t_target	ret;
	int		i;
	int		diff;
	...
	while (head)
	{
		if ((stack_b - head->value) < 0 && ft_diff(stack_b, head->value) < diff)
		{
			...
			ret = ft_count_target(head->value, i, list_size);
		}
		i++;
		head = head->next;
	}
...
t_pair	ft_findpair(t_utils *utils)
{
	...
	while (head)
	{
		...
		{
			pair.stack_b = ft_count_target(head->value, i, utils->count_b);
